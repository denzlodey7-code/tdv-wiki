...
describe("GET /api/docs/[slug]", () => {
  it("возвращает метаданные и контент", async () => {
    putMdx(
      "test-page",
      { title: "Test Page", section: "Demo" },
      "# Hello\n\nBody text",
    );

    const res = await getDoc(new Request("http://localhost"), {
      params: makeParams("test-page"),
    });
...
describe("PUT /api/docs/[slug]", () => {
  it("обновляет существующий файл", async () => {
    putMdx(
      "editable",
      { title: "Old Title", section: "S", order: 1, slug: "editable" },
      "Old body",
    );
